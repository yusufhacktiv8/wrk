...
exports.update = function update(req, res) {
  const uploadForm = req.body;
  models.Upload.update(uploadForm, {
    where: { id: req.params.uploadId },
  })
    .then((result) => {
      res.json(result);
...
const insertOmzet = (year, month, workbook) =>
  new Promise((resolve, reject) => {
    ...
    let prognosa = parseFloat(
      worksheet.getCell(OMZET_PROGNOSA_CELL).value.result
    );
...
const insertSales = (year, month, workbook) =>
  new Promise((resolve, reject) => {
    ...
    let prognosa = parseFloat(
      worksheet.getCell(SALES_PROGNOSA_CELL).value.result
    );
...
const insertNetProfit = (year, month, workbook) =>
  new Promise((resolve, reject) => {
    ...
    let prognosa = parseFloat(
      worksheet.getCell(NET_PROFIT_PROGNOSA_CELL).value.result
    );
    ...
    .then(() => {
      ...
      if (sheetType === "HEAD OFFICE") {
        ...
        if (role === "ADMIN") {
          ...
          promises.push(
            UploadProject.insertProject(year, month, projectCode, workbook)
          );
        ...
        if (role === "ADMIN") {
          ...
          promises.push(
            UploadScore.insertScores(year, month, projectCode, workbook)
          );
        } else {
          res.send("Unauthorized", 403);
          return;
